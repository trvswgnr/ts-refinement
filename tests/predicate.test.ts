...
describe("predicate parsing and subject inference", () => {
  ...
  it("rejects every global removed from the prior allowlist", () => {
    for (const name of removedGlobals) {
      expect(parsePredicate(ts, name).diagnostics[0]?.code, name).toBe(1000002);
    }
  });
  ...
  it.each([
    ...
    expect(parsePredicate(ts, source).diagnostics[0]?.code).toBe(1000002);
  });
  ...
  it("retains ambiguous free identifiers for contextual capture resolution", () => {
    expect(parsePredicate(ts, "n >= MIN_AGE").diagnostics[0]?.code).toBe(1000002);
  ...
  it("rejects ambiguity, malformed syntax, and mutation", () => {
    expect(parsePredicate(ts, "n > min").diagnostics[0]?.code).toBe(1000002);
    expect(parsePredicate(ts, "n >").diagnostics[0]?.code).toBe(1000000);
    expect(parsePredicate(ts, "n += 1").diagnostics[0]?.code).toBe(1000000);
    expect(parsePredicate(ts, "n++").diagnostics[0]?.code).toBe(1000000);
    expect(parsePredicate(ts, "import('x')").diagnostics[0]?.code).toBe(1000000);
    expect(parsePredicate(ts, "import.meta.url").diagnostics[0]?.code).toBe(1000000);
    expect(parsePredicate(ts, "this.value").diagnostics[0]?.code).toBe(1000000);
    expect(parsePredicate(ts, "delete n.value").diagnostics[0]?.code).toBe(1000000);
    expect(parsePredicate(ts, "if (n) true").diagnostics[0]?.code).toBe(1000000);
    expect(parsePredicate(ts, "(() => { return true; })() ").diagnostics[0]?.code).toBe(1000000);
  });

  it("does not treat host-environment identifiers as standard globals", () => {
    expect(parsePredicate(ts, "Buffer.isBuffer(n)").diagnostics[0]?.code).toBe(1000002);
